...
let testsFile = 'node_modules/testcafe/lib/utils/handle-errors.js';

let testsContent;

try {
    testsContent = fs.readFileSync(testsFile).toLocaleString();
}
catch {
    try {
        testsFile = '../../' + testsFile;
        ...
        console.log('Failed to find file by path: ' + testsFile);
    }
}
...
if (testsContent) {
    ...
    const v2016Replaced = `const _runningTests = {};
    const runningTests = new Proxy(_runningTests, {
        set: function (target, key, value) {
            if(value.ctx && value?.test?.meta) {
                try {
                    ...
                    value.ctx.runId = typeof value.testRunCtx?.runId === 'number' ? value.testRunCtx.runId : 
                        reporters.findIndex(rep => rep.testsInfo.find(inf => 
                            value.test.meta.id === inf.meta.id &&
                            value.test.meta.name === inf.meta.name &&
                            value.test.meta.fixtureName === inf.meta.fixtureName 
                            ));
                    value.testRunCtx.runId = value.ctx.runId;
    ...
    if (oldVersions.some((v) => testsContent.includes(v))) {
        fs.writeFileSync(
            testsFile,
            oldVersions.reduce(
                (val, ver) => val.replace(ver, v2016Replaced),
                testsContent
            )
        );
    }
...
try {
    ...
    concurencyBlockFile = '../../' + concurencyBlockFile;
    try {
        concurencyContent = fs.readFileSync(concurencyBlockFile).toLocaleString();
    }
    catch {
        console.log('Failed to find file by path: ' + concurencyBlockFile);
    }
}

if (concurencyContent) {
...
try {
    ...
    failedStepMarkFile = '../../' + failedStepMarkFile;
    try {
        failedTestContent = fs.readFileSync(failedStepMarkFile).toLocaleString();
    } 
    catch {
        console.log('Failed to find file by path: ' + failedStepMarkFile);
    }
}

if (failedTestContent && !failedTestContent.includes(failedTestReplaceWith))
